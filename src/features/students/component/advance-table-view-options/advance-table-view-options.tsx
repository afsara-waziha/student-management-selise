...
/**
 * A component for managing column visibility options in a Students table. This component provides a dropdown menu
 * that allows users to toggle the visibility of individual columns, as well as toggle all columns at once.
 * It ensures that certain columns cannot be hidden (such as `name`, `studentId`, `email`, and `grade`),
...
export function AdvanceTableViewOptions<TData>({
  ...
  const handleToggleAll = () => {
    ...
    table.getAllColumns().forEach((column) => {
      if (column.getCanHide() && !['name', 'studentId', 'email', 'grade'].includes(column.id)) {
